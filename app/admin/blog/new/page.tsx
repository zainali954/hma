...
export default function NewBlogPostPage() {
  ...
  const validate = (checkStatus: "draft" | "published" | "scheduled"): boolean => {
    const newErrors: Record<string, string> = {};
    if (!title.trim()) newErrors.title = "Title is required";
    if (!slug.trim()) newErrors.slug = "Slug is required";
    if (!content.trim()) newErrors.content = "Content is required";

    if (checkStatus === "scheduled" && !publishDate) {
      newErrors.publishDate = "Publish date is required for scheduled posts";
    }

    // Length warnings (not blocking)
    if (seoTitle && (seoTitle.length < 30 || seoTitle.length > 70)) {
      newErrors.seoTitle = "SEO title should be 30–70 characters";
    }
    if (metaDescription && (metaDescription.length < 120 || metaDescription.length > 165)) {
      newErrors.metaDescription = "Meta description should be 120–165 characters";
    }
    ...
    try {
      const res = await fetch("/api/blog", {
        ...
        body: JSON.stringify({
          ...
          published: publish,
          status: finalStatus,
          publishDate: finalStatus === "scheduled" ? publishDate : null,
          featured,
          seoTitle,
          metaDescription,
          focusKeyword,
